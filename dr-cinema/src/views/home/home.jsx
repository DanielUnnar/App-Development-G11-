...
import { useGetCinemasQuery } from '../../services/APIservice';
...
export function HomeScreen({ navigation, route }) {
  ...
  const { data: cinemasData, error, isLoading } = useGetCinemasQuery();
  const [cinemas, setCinemasData] = useState([]);

  useEffect(() => {
    if (cinemasData) {
      const sortedCinemas = cinemasData.cinemas.slice().sort((a, b) => a.name.localeCompare(b.name));
      setCinemasData(sortedCinemas);
    }
  }, [cinemasData]);
